...
/**
 * Mock-auth dev control: toggle the active role. Each option is a tiny form
 * posting to the `switchRole` server action — no client JS required.
 */
          ...
          <form key={role} action={switchRole}>
            ...
            <button
              ...
              className={`rounded-md px-3 py-1.5 text-xs font-medium transition ${
                isActive ? "bg-white text-brand-700 shadow-sm" : "text-slate-500 hover:text-slate-800"
              }`}
            >
              {ROLE_LABELS[role]}
            </button>
